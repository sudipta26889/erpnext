...
- **`Item.reorder_levels`** (the `Item Reorder` child table) — closed for **both spellings** now. `documents.FIELD_FORBIDDEN_KEYS` closed the parent-payload spelling (`create_document`/`update_document("Item", ..., {"reorder_levels": [...]})`) in this pass, but `FIELD_FORBIDDEN_KEYS` is keyed on the *parent* doctype name, and nothing on the write path checked `meta.istable` — so the child-table spelling, `update_document("Item Reorder", <row name>, {...})`, stayed open (row names are readable via `get_document("Item", ...)`) until a follow-up pass (2026-08-07) added an unconditional `meta.istable` refusal to `documents._assert_writable_doctype`, the chokepoint shared by all five write tools. That same gap let `update_document("Has Role", <row>, {"role": "System Manager"})` write straight into `tabHas Role` — privilege escalation around the `User`/`Role` deny-list entries above — which is what made the istable guard critical rather than cosmetic. Read by `erpnext/stock/reorder_item.py`'s daily `reorder_item` scheduled job, which ends in `mr.submit()` for whatever Purchase/Transfer/Material Issue/Manufacture Material Request it builds off each row's `warehouse_reorder_level`/`warehouse_reorder_qty`/`material_request_type`. `Item` has no `company` field (passes `assert_scopable` unchallenged) and is not itself in `WRITE_FORBIDDEN_DOCTYPES`, and `max_document_value` is never consulted because the cap only ever runs against a document this tool surface itself inserts/saves/submits — never the Material Request the scheduler builds later, off a different document, on its own schedule.

The istable guard closes the *child-table* shape of the gap structurally (no child doctype is writable at all, regardless of which field), but it does not change the underlying structural limitation above: a name-based denylist (`WRITE_FORBIDDEN_DOCTYPES`, `FIELD_FORBIDDEN_KEYS`) can only ever enumerate doctypes and fields someone has already thought to check. The correct long-term shape is still a **write allowlist** of vetted business doctypes, not an ever-growing denylist.
